...
describe('SignupScreen (AuthScreen mode=signup)', () => {
  ...
  it('renders signup fields', () => {
    ...
    expect(getByPlaceholderText('מיקוד *')).toBeTruthy();
  ...
  it('validates password strength rules', async () => {
    ...
    fireEvent.changeText(getByPlaceholderText('מס\' טלפון *'), '0501234567');
    fireEvent.changeText(getByPlaceholderText('מיקוד *'), '1234567');
    fireEvent.changeText(getByPlaceholderText('כתובת *'), 'Some Street 1');
    fireEvent.changeText(getByPlaceholderText('ת. זהות *'), '123456789');
